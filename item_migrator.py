...
class ItemMigrator:
    ...
    def create_widgets(self):
        ...
        ttk.Label(filter_frame, text="Name contains:").grid(row=0, column=5, sticky="w")
        self.name_entry = ttk.Entry(filter_frame, width=15)
        self.name_entry.grid(row=0, column=6, padx=2)

        ttk.Label(filter_frame, text="Subtype:").grid(row=0, column=7, sticky="w")
        ...
        self.subtype_combo.grid(row=0, column=8, padx=2)

        ttk.Label(filter_frame, text="Quality:").grid(row=0, column=9, sticky="w")
        ...
        columns = ("Select", "ID", "Name", "Quality", "Type", "Subtype", "Level")
        self.items_tree = ttk.Treeview(table_frame, columns=columns, show="headings", height=18)
        self.items_tree.heading("Select", text="Select")
        ...
        self.items_tree.column("Type", width=80, anchor="center")
    ...
    def load_items(self):
        ...
        try:
            ...
            SELECT entry, class, subclass, name, displayid, Quality, ItemLevel, stackable, BuyPrice, SellPrice, RequiredLevel, \
                   ...
                   dmg_min1, dmg_max1, delay, armor, holy_res, fire_res, nature_res, frost_res, shadow_res, arcane_res, description
            FROM item_template 
            ...
            # Apply filters in Python for name, subtype, quality
            filtered_results = []
            for row in results:
            ...
            for row in results:
                ...
                item_id = self.items_tree.insert("", "end", values=(
                    "☐", item_data['entry'], item_data['name'], quality_name, item_data['class'], subtype_name, item_data['ItemLevel']
    ...
    def show_item_details(self, item_data):
        ...
        details += f"Type: {item_data['class']}\n"
